...
parser = argparse.ArgumentParser(
    description='Generate a hyperbola (resorbed rod).',
...
if args.distance <= 0:
    os.sys.exit('[ERROR] Resorption distance cannot be less than or equal to zero ({} <= 0)'.format(args.distance))

print('Computing a and b')
a = float(2.0/args.distance)
#b = np.sqrt( (args.length**2 / args.distance**2 + 1) / args.radius**2)
b = np.sqrt( ( (args.length / args.distance)**2 + 1) / args.radius**2)
